...
export const updateAlbum = async (req:Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (titulo) albumData.titulo = titulo
        if (anio) albumData.anio = anio
        if (genero) albumData.genero = genero
        if (artistaId) albumData.artistaId = artistaId
        if (artista) albumData.artista = artista
